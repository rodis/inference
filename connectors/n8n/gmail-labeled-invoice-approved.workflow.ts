/**
 * Connector: Gmail — invoice approval label -> raw_sensors  (ADR 0008 + ADR 0012)
 *
 * Watches the label that means "I have checked these figures and I approve them", and POSTs
 * each matching message to the ingest gateway. A near-copy of gmail-labeled-parking: same three
 * nodes, same contract, one label and one event name different.
 *
 * WHY A LABEL AND NOT AN LLM. ADR 0012 puts semantic classification in the reconciler, for mail
 * whose meaning has to be *read* (telling a submitted payment from a merely processed one).
 * Approval is not that: applying the label IS the human decision, already made and already
 * unambiguous. Interpreting it would add a way to be wrong about something we were told.
 *
 * The reconciler correlates the resulting event back to its cycle on `subject`, which the
...
const LABEL = 'aware/invoice-approved';
const EVENT_NAME = 'email_labeled_invoice_approved';
...
const CANONICAL_BODY =
  ...
  // Kept for the same reason as elsewhere: whatever is dropped here cannot be re-fetched
  // by the runtime. For an approval mail the snippet is evidence of WHAT was approved.
...
export default workflow('connector-gmail-invoice-approved', 'Connector: gmail — invoice approval label')
