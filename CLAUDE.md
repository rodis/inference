...
> **⚠️ The DEPLOYED runtime is now Quix Streams (ADR 0004), not the threaded runtime below.** As of 2026-06-27 the `inference-runtime` image runs [`workers/runtime/quix_main.py`](workers/runtime/quix_main.py) → [`inference.runtime.quix`](src/inference/runtime/quix.py): one Quix `Application`, per-`(definition, entity)` window in partition-local Quix `State` (RocksDB + changelog, **no Redis**), envelopes produced straight to `high_level_events` (**Vector is no longer in the emit path** — only ingest + Neon persist), recursion resolved in-process. The threaded `RuntimeSupervisor`/`KafkaStreamHandler`/`WeightedWindowEngine`/Redis stack described below is **superseded** but kept in the repo (and `workers/runtime/main.py` for rollback). See [`doc/adr/0004-scaling-model.md`](doc/adr/0004-scaling-model.md). Docs below (and ADR 0003) still describe the threaded model and need reconciliation.

> **ADR 0003 (threaded runtime — now superseded by 0004).** An inference event is defined as data — a YAML file in [`events/`](events/) — and a single generic **runtime** ([`workers/runtime/main.py`](workers/runtime/main.py)) loads every definition and runs one handler per definition in one process/pod. This replaced the old one-pod-per-event model where identity was the directory name. See [`doc/adr/0003-dynamic-event-runtime.md`](doc/adr/0003-dynamic-event-runtime.md).
