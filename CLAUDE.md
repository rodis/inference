...
- **`to_event(name, decision, user_id)`** — runtime-side shaping in one step: mint the full `high_level_events` event record from an engine `Decision`. The top-level wrapper is **identical to the one Vector mints for raw events** (`name`, `source_app`, `source_type`, `timestamp`, `message`) so every Kafka topic carries the same shape; `source_type` records the entry mechanism (`"kafka"` for derived, `"http_server"` for raw) and is metadata only — the persister drops it, it never reaches Neon. The `message` holds the per-event `id` (no top-level "envelope" id anymore — minted here for derived, by Vector for raw), `derived_from` lineage, the entity `user_id`, and the derived-only `inference_type`/`processed_at`. The old `decide → finalize → Vector-re-wrap` hop is gone — we produce straight to Kafka — so engines only decide; all shaping lives here.
...
**Identity.** The definition `name` (snake_case) is the emitted event `name`/`inference_type` and the sink-routing key. The whole runtime shares **one** Kafka consumer group (`QUIX_CONSUMER_GROUP`, default `inference-quix-runtime-v1`) — *not* a group per event (that was the threaded model).
