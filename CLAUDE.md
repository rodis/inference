...
- **`finalize(name, decision, user_id)` + `to_envelope(name, message)`** — runtime-side shaping: turn an engine `Decision` into the `high_level_events` message (`sources`/`evidence`/`derived_from` from contributors, stamps the entity `user_id`) and wrap it in the Envelope (`envelope_id`, `source_type="inference_quix"`, …). Engines don't shape envelopes.
